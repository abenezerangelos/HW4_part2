...
class Name(Expr):
    ...
    def eval(self,psstacks):
        ...
        if self.var_name[0]=="/":
            ...
            rt_obj=psstacks.lookup(self.var_name)

            if isinstance(rt_obj,CodeArray):
                rt_obj.apply(psstacks)
            else:
                psstacks.opPush(rt_obj)
...
class DictConstant(Value):
    """A dictionary constant. Attempting to apply an `dictionary constant` will give an error.
...
class CodeArray(Value):
    ...
    def apply(self, psstacks):
        """ TODO in part2 """
        if len(psstacks.dictstack) == 0:
            staticlink = 0
        else:
            staticlink = self.staticlinker(psstacks)
    ...
    def staticlinker(self,psstacks):

              staticlink=self.staticlinkerhelper(psstacks,len(psstacks.dictstack)-1)

              return staticlink

    def staticlinkerhelper(self,psstacks,ind):

         starter=psstacks.dictstack[ind][0]
         if starter==ind:
             return ind
         return self.staticlinkerhelper(psstacks,starter)
